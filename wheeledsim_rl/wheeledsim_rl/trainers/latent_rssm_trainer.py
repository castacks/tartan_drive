...
class LatentProbRSSMTrainer(LatentWorldModelsTrainer):
    """
    ...
    def __init__(
        ...
        """
        super(LatentProbRSSMTrainer, self).__init__(env, policy, model, opt, replay_buffer, collector, model_itrs, contrastive_scale, n_steps, n_eval_steps, batch_size, discount, reward_scale, epochs, steps_per_epoch, initial_steps, eval_dataset)
        self.ema_tau = ema
    ...
    def update(self, batch):
        ...
        latent_prior = out['latent_prior']

        self.logger.record_tensor("State std", preds['state'].scale.detach(), prefix="Debug")
        self.logger.record_tensor("Latent state mean", latent_prior.mean.detach(), prefix="Debug")
        self.logger.record_tensor("Latent state std", latent_prior.scale.detach(), prefix="Debug")

        #NLL
        state_log_prob = preds['state'].log_prob(targets['state']).sum(dim=-1).clamp(-1000., 1000.)

        losses = {}
        losses["NLL Loss"] = -state_log_prob.mean()

        self.logger.record_tensor("Target State Log Prob", state_log_prob.detach(), prefix="Debug")

        latent_state_posterior = self.model.get_latent_posterior(batch['next_observation'], hidden_states)
        ...
        for k in self.model.obs_keys:
            r_loss = (targets[k] - preds[k]).pow(2).mean()
            losses["{} Reconstruction Loss".format(k)] = r_loss * self.contrastive_scale

        #Posterior KL
#        kld = torch.distributions.kl.kl_divergence(latent_state_posterior, latent_prior)

        kl1 = torch.distributions.kl.kl_divergence(latent_state_posterior, torch.distributions.Normal(loc=latent_prior.mean.detach(), scale=latent_prior.scale.detach()))
        kl2 = torch.distributions.kl.kl_divergence(torch.distributions.Normal(loc=latent_state_posterior.mean.detach(), scale=latent_state_posterior.scale.detach()), latent_prior)

#        kl1 = torch.distributions.kl.kl_divergence(latent_state_posterior, StraightThroughOneHotCategorical(probs=latent_prior.probs.detach()))
#        kl2 = torch.distributions.kl.kl_divergence(StraightThroughOneHotCategorical(probs=latent_state_posterior.probs.detach()), latent_prior)

        balanced_kl = (0.2*kl1 + 0.8*kl2).sum(dim=-1) * 0.01

        losses["KL Posterior Loss"] = balanced_kl.mean()
...
#        if loss.item() > 1000. and self.current_epoch > 10:
...
class BilinearLatentProbRSSMTrainer(LatentProbRSSMTrainer):
    """
...
#            latent_sensor_preds = self.model.encode_observation(batch['next_observation'])
            latent_sensor_preds = self.model.predict(dict_map(batch['observation'], lambda x:x.flatten(end_dim=1)) , batch['action'].view(-1, self.model.act_dim).unsqueeze(1), return_info=True)
