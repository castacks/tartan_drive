...
class LatentWorldModelsTrainer(OffPolicyRLAlgorithm):
    """
    Unlike the regular world models trainer, which uses a reconstruction loss, this one uses the encoder to enforce that the encodings in latent space match.
    """
    def __init__(
            self,
            env,
        ...
        """
        super(LatentWorldModelsTrainer, self).__init__(env, discount, reward_scale, epochs, steps_per_epoch, initial_steps, replay_buffer, model_itrs, batch_size, model.input_normalizer)
        self.policy = policy
        self.model = model
    ...
    def evaluate(self):
        ...
        for traj in self.eval_dataset:
            ...
            with torch.no_grad():
                preds = self.model.predict(curr, traj['action'][t:t+self.n_eval_steps].unsqueeze(0), return_info=False)['state'].squeeze()
    ...
    def update(self, batch):
        ...
        latent_err = (latent_preds - latent_sensor_preds) * self.contrastive_scale

        losses['Latent Loss'] = latent_err.pow(2).mean()

#        if losses['Latent Loss'].item() > 1.:
